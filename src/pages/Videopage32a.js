...
//     image: "",
//     voicetext: "Text-to-speech feature is now available on relatively any website or blog. It's a game changer that you can listen to the content instead of reading it. Especially effective for people with visual or cognitive impairments or people who are on the go. I came up with the idea to implement it for my blog, so this is how I started doing a research on this topic which ended up being a tutorial for you. So in this tutorial, we will go through the process of building a text-to-speech component in React. We will use the Web Speech API to implement the text-to-speech functionality.",
//     duration: 62,
//     type: "text",
//   },
//   {
...
const VideoPage = () => {
  ...
  useEffect(() => {
    const updateVoices = () => {
      ...
      // }
      setSelectedVoice(global1.selvoice);
    };
  ...
  // // Origin method which currently used
  ...
  // Handle for speak text normal without chunk
  const speakText = (text) => {
    if (
      speechSupported &&
      selectedVoice &&
  ...
  // // Handle for speak text with chunk
  // const speakText = (text) => {
  //   if (
  //     speechSupported &&
  //     selectedVoice &&
  ...
  //     const utterances = [];
  //     const chunkSize = 140; // Chrome tends to have issues with long texts, so break it into smaller chunks.
  //     let start = 0;
  
  //     // Split the text into smaller chunks and create utterances
  ...
  //       utterance.onend = () => {
  //         // Set isSpeaking to false only after the last chunk is spoken
...
// // Handle for speak text without chunk 2nd method
  // const speakText = (text) => {
  //   if (
  //     speechSupported &&
  //     selectedVoice &&
  ...
  //     // Optional: Adjust the rate slightly if Chrome cuts off
  //     utterance.rate = 0.9; // Slightly slower to prevent cut-off
  
  //     utterance.onboundary = (event) => {
  //       // This event fires during speech, so we can use it to ensure it's still progressing
  //       console.log(
  //         `Speaking word: "${text.substring(event.charIndex)}"`
  //       );
  //     };
  ...
  //       setIsSpeaking(false); // Handle errors gracefully
  //     };
